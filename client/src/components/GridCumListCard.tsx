...
const GridCumListCard: React.FC<Props> = ({ anime, importedData }) => {
    const mainColor = anime.coverImage?.color || '#e0833a';

    return (
        <div className="flex relative rounded-xl overflow-hidden bg-[#11161d]/80 backdrop-blur-md p-4 gap-5 transition-transform hover:scale-[1.02] shadow-lg h-[280px] sm:h-[310px]">
            {/* Left Column */}
            <div className="flex flex-col gap-3 w-[130px] sm:w-[150px] shrink-0 h-full">
            ...
            <div className="flex flex-col flex-1 py-1 min-w-0 min-h-0">
                ...
                >
                    <h3 className="font-bold text-xl sm:text-2xl text-white truncate font-sans">
                ...
                {(anime.title?.native || anime.title?.romaji) && (
                    <span
                        className="text-sm italic font-medium mb-3 truncate shrink-0 pr-1"
                ...
                <div
                    className="text-[#8ba0b2] text-[13px] sm:text-sm leading-relaxed overflow-y-auto custom-scrollbar-hidden mb-auto pr-2"
                    dangerouslySetInnerHTML={{ __html: anime.description ? anime.description.replace(/<br\s*\/?>/gi, ' ').replace(/[\r\n]+/g, ' ') : 'No synopsis available.' }}
                />

                {/* Info Row */}
                <div className="flex items-center gap-4 text-[#8ba0b2] text-sm font-semibold mt-4 mb-3 flex-wrap shrink-0">
                    ...
                    {anime.averageScore && (
                        <div className="flex items-center gap-1.5">
                            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 flex-shrink-0 -ml-[1.5px]" viewBox="0 0 24 24" fill="currentColor" style={{ color: 'currentColor' }}>
                    ...
                    {anime.popularity && (
                        <div className="flex items-center gap-1.5">
                            ...
                            <span>{anime.popularity >= 1000 ? (anime.popularity / 1000).toFixed(1) + 'k' : anime.popularity}</span>
                        </div>
                    )}
                    {anime.episodes && (
                        <div className="flex items-center gap-1.5">
                ...
                <div className="flex flex-wrap gap-1.5 shrink-0">
                    {anime.genres?.slice(0, 3).map((g: string) => (
                        <span key={g}
                            className="text-[11px] font-bold backdrop-blur-sm px-2 py-1 rounded-full"
                            style={{
