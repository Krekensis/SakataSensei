...
const GridCumListCard: React.FC<Props> = ({ anime }) => {
    return (
        <a 
        ...
        >
            ...
            <div className="w-[100px] sm:w-[120px] flex-shrink-0 relative h-full rounded shadow-sm overflow-hidden z-10">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col">
                        ...
                        <div className="text-xs font-semibold text-[#8ba0b2] mt-1 flex items-center gap-2 flex-wrap">
                            {anime.studios?.nodes?.[0]?.name && (
                                <span className="text-[#3db4f2] px-1.5 py-0.5 bg-[#151f2e] rounded-sm">{anime.studios.nodes[0].name}</span>
                            )}
                            ...
                            {anime.episodes && <span className="px-1.5 py-0.5">{anime.episodes} eps</span>}
                        </div>
                    </div>
                ...
                <div className="mt-auto pt-2 flex flex-wrap gap-2 shrink-0">
                    {anime.genres?.slice(0, 3).map((g: string) => (
                        <span key={g} 
                            ...
                            style={{
                                backgroundColor: anime.coverImage?.color ? `${anime.coverImage.color}26` : 'rgba(21, 31, 46, 0.5)',
                                color: anime.coverImage?.color || '#8ba0b2'
                            }}
                        >
                            {g}
                        </span>
