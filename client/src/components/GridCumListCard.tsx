    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col gap-2 sm:gap-3 w-[90px] min-[400px]:w-[110px] sm:w-[150px] shrink-0 h-full">
                <div className="relative w-full flex-1 rounded-lg overflow-hidden shadow-md group">
            ...
            <div className="flex flex-col flex-1 py-1 min-w-0 min-h-0">
                ...
                {/* Info Row */}
                <div className="flex items-center gap-3 sm:gap-4 text-[#acc7dd] text-[11px] sm:text-sm font-semibold mt-2 sm:mt-4 mb-2 sm:mb-3 flex-wrap shrink-0">
                    <div className="flex items-center gap-1.5">
                        <div className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${anime.status === 'RELEASING' ? 'bg-lime-400' :
                            anime.status === 'FINISHED' ? 'bg-blue-400' :
                                anime.status === 'CANCELLED' ? 'bg-red-400' :
                                    'bg-orange-300'
                            }`}></div>
                        <span className="sm:hidden">{anime.format === 'MOVIE' ? 'MOV' : (anime.format === 'TV_SHORT' ? 'TV_S' : (anime.format || 'TV'))}</span>
                        <span className="hidden sm:inline">{anime.format === 'TV_SHORT' ? 'TV_S' : (anime.format || 'TV')}</span>
                    </div>
                    {anime.startDate?.year && <span>{anime.startDate?.year}</span>}
                    {anime.averageScore && (
                        <div className="flex items-center gap-1">
                            <SVG name="star" size="" className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0 -ml-[1px]" style={{ color: 'currentColor' }} />
                            <span>{anime.averageScore}%</span>
                        </div>
                    )}
                    {anime.popularity && (
                        <div className="flex items-center gap-1">
                            <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0" viewBox="0 0 640 512" fill="currentColor" style={{ color: 'currentColor' }}>
