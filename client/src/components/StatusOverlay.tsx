...
const StatusOverlay: React.FC<StatusOverlayProps> = ({ anime, importedData }) => {
    ...
    if (isLoading) {
        return (
            <div className="absolute top-2 right-2 bg-[#11161d]/90 p-1.5 rounded-lg opacity-100 transition-opacity z-10 pointer-events-none shadow-md">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 animate-spin" style={{ color: animeColor }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    if (currentStatus === 'COMPLETED') {
        return (
            <div className="absolute top-2 right-2 bg-[#11161d]/90 p-1.5 rounded-lg opacity-90 transition-opacity z-10 pointer-events-none shadow-md">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" style={{ color: animeColor }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
    ...
    if (currentStatus === 'PLANNING' || currentStatus === 'CURRENT' || currentStatus === 'DROPPED' || currentStatus === 'ON_HOLD') {
        ...
        return (
            <div className="absolute top-2 right-2 bg-[#11161d]/90 p-1.5 rounded-lg opacity-90 transition-opacity z-10 pointer-events-none shadow-md">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" style={{ color: animeColor }} viewBox="0 0 24 24" fill="currentColor">
